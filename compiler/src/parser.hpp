...
private:
    ...
    std::shared_ptr<ASTNode> parseClass() {
        int line = current().line, col = current().col;
        advance(); // skip 'class'
        auto nameToken = expect(TokenType::IDENTIFIER, "Expected class name");
        // Simple class - body as block for now
        parseBlock(); // consume the class body
    ...
    std::shared_ptr<ASTNode> parseCall() {
        ...
        while (true) {
            if (match(TokenType::LPAREN)) {
                ...
                auto prop = expect(TokenType::IDENTIFIER, "Expected property name after '.'");
                expr = std::make_shared<DotAccessNode>(std::move(expr), prop.value, current().line, current().col);
